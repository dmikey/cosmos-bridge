...
import { decimals } from "./src/banker/monetary/denoms";
import BigNumber from "bignumber.js";

// npm run build:contracts
import ethContract from "./build/contracts/wAKT.json";
...
export class ETHMinter {
  ...
  mintOnly = async (address: string) => {
    ...
    const txFeeEstimate: any = new BigNumber(gasLimit * (gasPrice as any));

    // send some money
    const mintValue: any = new BigNumber(
      (parsedReply.tender / decimals.uakt) * decimals.wakt
    );
...
const minter = new ETHMinter({
  logger: Pino(),
});
minter.mintOnly("0xCcC815F4ba81fE6DC34dcE67B2de701efdf1f7FA");
